...
static inline int zng_tr_tally_dist(deflate_state *s, uint32_t dist, uint32_t len) {
    ...
    s->matches++;
    dist--;
    Assert(dist < MAX_DIST(s) && (uint16_t)d_code(dist) < (uint16_t)D_CODES, "zng_tr_tally: bad match");
...
#define FLUSH_BLOCK_ONLY(s, last)                                                                          \
    {                                                                                                      \
        zng_tr_flush_block(s, (s->block_start >= 0 ? (char *)&s->window[(unsigned)s->block_start] : NULL), \
                           (uint32_t)((int)s->strstart - s->block_start), (last));                         \
