...
int Z_INTERNAL inflate_ensure_window(struct inflate_state *state) {
    ...
    if (state->window == NULL) {
        ...
        state->window = (unsigned char *)ZALLOC_WINDOW(state->strm, wsize + state->chunksize, 2);
        if (state->window == NULL)
            return Z_MEM_ERROR;
            ...
            if (state->wrap) {
                ...
                if (INFLATE_NEED_CHECKSUM(strm) && out) {
                    if ((state->wrap & 2) && x86_cpu_has_pclmulqdq)
                        strm->adler = state->check = crc_fold_512to32(state->crc0);
                }
#endif
                out = left;
                if ((state->wrap & 4) && (
...
int32_t Z_EXPORT PREFIX(inflateCopy)(PREFIX3(stream) *dest, PREFIX3(stream) *source) {
    ...
    if (state->window != NULL) {
        ...
        window = (unsigned char *)ZALLOC_WINDOW(state->strm, wsize + state->chunksize, 2);
        if (window == NULL) {
    ...
    if (window != NULL) {
        wsize = 1U << state->wbits;
        memcpy(window, state->window, (wsize + state->chunksize) * 2);
    }
    copy->window = window;
