...
/* LIT_MEM uses separate distance/length buffers instead of the overlaid sym_buf.
   This uses ~20% more memory but is faster on platforms without fast unaligned access.

   By default, LIT_MEM is only enabled when OPTIMAL_CMP < 32 (slow unaligned access).
   Define LIT_MEM to force separate buffers, or NO_LIT_MEM to force sym_buf usage. */
...
#   define max_insert_length  max_lazy_match
    ...
    int32_t padding1[1];
                ...
                /* Cacheline 3 */
    uint8_t ALIGNED_(16) padding4[52];
