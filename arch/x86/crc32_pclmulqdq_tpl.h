...
#define PCLMULQDQ_CHORBA_MIN_LEN (512 + 64 + 16*8)

static inline void fold_state_1(__m128i *xmm_crc0, __m128i *xmm_crc1, __m128i *xmm_crc2, __m128i *xmm_crc3, const __m128i xmm_fold4) {
...
static inline void fold_state_2(__m128i *xmm_crc0, __m128i *xmm_crc1, __m128i *xmm_crc2, __m128i *xmm_crc3, const __m128i xmm_fold4) {
...
static inline void fold_state_3(__m128i *xmm_crc0, __m128i *xmm_crc1, __m128i *xmm_crc2, __m128i *xmm_crc3, const __m128i xmm_fold4) {
    __m128i x_low0  = _mm_clmulepi64_si128(*xmm_crc0, xmm_fold4, 0x01);
    __m128i x_high0 = _mm_clmulepi64_si128(*xmm_crc0, xmm_fold4, 0x10);
    __m128i x_low1  = _mm_clmulepi64_si128(*xmm_crc1, xmm_fold4, 0x01);
    __m128i x_high1 = _mm_clmulepi64_si128(*xmm_crc1, xmm_fold4, 0x10);
...
static inline void fold_state_4(__m128i *xmm_crc0, __m128i *xmm_crc1, __m128i *xmm_crc2, __m128i *xmm_crc3, const __m128i xmm_fold4) {
    __m128i x_low0  = _mm_clmulepi64_si128(*xmm_crc0, xmm_fold4, 0x01);
    __m128i x_high0 = _mm_clmulepi64_si128(*xmm_crc0, xmm_fold4, 0x10);
    __m128i x_low1  = _mm_clmulepi64_si128(*xmm_crc1, xmm_fold4, 0x01);
    __m128i x_high1 = _mm_clmulepi64_si128(*xmm_crc1, xmm_fold4, 0x10);
...
/* 512-bit folding block: enter wide mode, process, exit wide mode */
static inline void fold_block_16(const uint8_t **src, uint8_t **dst, size_t *len,
    __m128i *xmm_crc0, __m128i *xmm_crc1, __m128i *xmm_crc2, __m128i *xmm_crc3, const int COPY) {
    *len -= 256;
    ...
    *src += 256;
    ...
    /* Promote: fold existing xmm state into first 64 bytes */
    ...
    /* Main loop: process 256 bytes per iteration */
    while (*len >= 256) {
        *len -= 256;
    ...
    /* Reduce: zmm_crc[0,1,2,3] -> zmm_crc0 */
...
/* 256-bit folding block: enter wide mode, process, exit wide mode */
static inline void fold_block_8(const uint8_t **src, uint8_t **dst, size_t *len,
    ...
    *src += 128;
    ...
    /* Promote: fold existing xmm state into first 32 bytes */
    ...
    /* Main loop: process 128 bytes per iteration */
    while (*len >= 128) {
        *len -= 128;
    ...
    /* Reduce: ymm_crc[0,1,2,3] -> ymm_crc0 */
    y_low0 = _mm256_clmulepi64_epi128(ymm_crc0, ymm_fold4, 0x01);
    y_high0 = _mm256_clmulepi64_epi128(ymm_crc0, ymm_fold4, 0x10);
    ymm_crc0 = z256_xor3_epi64(y_low0, y_high0, ymm_crc1);

    y_low0 = _mm256_clmulepi64_epi128(ymm_crc0, ymm_fold4, 0x01);
    y_high0 = _mm256_clmulepi64_epi128(ymm_crc0, ymm_fold4, 0x10);
    ymm_crc0 = z256_xor3_epi64(y_low0, y_high0, ymm_crc2);

    y_low0 = _mm256_clmulepi64_epi128(ymm_crc0, ymm_fold4, 0x01);
    y_high0 = _mm256_clmulepi64_epi128(ymm_crc0, ymm_fold4, 0x10);
    ymm_crc0 = z256_xor3_epi64(y_low0, y_high0, ymm_crc3);

    /* Exit: ymm_crc0 -> xmm_crc[0, 1, 2, 3] */
    *xmm_crc0 = _mm256_castsi256_si128(ymm_crc0);
    *xmm_crc1 = _mm256_extracti128_si256(ymm_crc0, 1);
    *xmm_crc2 = _mm_setzero_si128();
    *xmm_crc3 = _mm_setzero_si128();

    /* Fold xmm_crc0 and xmm_crc1 into xmm_crc2 and xmm_crc3 */
    __m128i x_low  = _mm_clmulepi64_si128(*xmm_crc0, xmm_fold4, 0x01);
    __m128i x_high = _mm_clmulepi64_si128(*xmm_crc0, xmm_fold4, 0x10);
    *xmm_crc2 = _mm_xor_si128(x_low, x_high);

    x_low  = _mm_clmulepi64_si128(*xmm_crc1, xmm_fold4, 0x01);
    x_high = _mm_clmulepi64_si128(*xmm_crc1, xmm_fold4, 0x10);
    *xmm_crc3 = _mm_xor_si128(x_low, x_high);
...
static inline void fold_block_chorba(const uint8_t **src, uint8_t **dst, size_t *len,
    ...
    if (!COPY)
        return;
#endif
    while (*len >= PCLMULQDQ_CHORBA_MIN_LEN) {
